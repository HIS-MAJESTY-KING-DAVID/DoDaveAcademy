...
>
> **Current behavioral parity score: 60% (2026-08-25).** This conservative score covers 21 capability groups and weights full parity as 1 point, partial/unverified parity as 0.5 points, and missing/unconfirmed parity as 0 points. The score increased because Block A admin recursive learning management passed its active scalar-contract acceptance gate; Block B has begun with lock-aware admin evaluation question CRUD. See `FEATURE_PARITY_PRODUCTION_READINESS.md` for the calculation and remaining gaps.
>
> **Deployment prerequisite:** Run `npm run db:migrate` against the target Supabase database before enabling `/dashboard/student/subject-chat`; the migration is additive and independent of the future payment provider.
...
| Payments and subscriptions | Course/subscription payment controllers and payment history | Payment checkout, init/webhook, subscription pages, history | Deferred provider parity | Existing initialization and validation remain; the replacement payment provider is intentionally deferred. Preserve server-authoritative amounts and callback verification when selected. |
...
| Chat and messaging | Chat, group chat, Firebase/push surfaces | Direct messenger plus per-learner subject-chat rooms and APIs | Partial parity implemented | Subject rooms, premium gating, unread state, edit/delete, realtime/polling, and mobile switching are present; teacher persona, WebSocket token compatibility, AI, migration rollout, and acceptance remain. |
...
| Admin learning CRUD | Chapters, lessons, quizzes, propositions, media | Admin course content page plus nested chapter/lesson/quiz/media/reorder routes | Full for active scalar quiz-option and media-reference contract | Standalone legacy Proposition scaffold remains separately tracked because learner quizzes consume `Quiz.proposition1..4`; continue broader admin catalog coverage. |
| Admin evaluations | Evaluation and evaluation-question controllers/templates | Admin list/detail/create/settings/actions plus lock-aware question CRUD routes and manager | Partial parity; create/settings/assignment/question slices shipped | Add correction/result controls, timing/scoring parity, and instructor/student role acceptance. |
